...
class TestCleaningNeuralNetwork(NeuralNetworkTester):
    ...
    def test_simple_clean(self):
        nn = 'neuron(1,i). neuron(2,i). neuron(3,o). output(3,up). edge(1,3). edge(1,2).'
        self.assert_cleaning(nn, NeuralNetwork.cleaned(nn))
        self.assert_cleaning(nn, 'neuron(1,i) neuron(3,o) edge(1,3) output(3,up)')

    def test_basic_memory(self):
        nn = 'neuron(1,i). neuron(2,o). edge(1,2). memwrite(2,0).'
        self.assert_cleaning(nn, NeuralNetwork.cleaned(nn))
        self.assert_cleaning(nn, 'neuron(1,i) neuron(2,o) edge(1,2) memwrite(2,0)')
